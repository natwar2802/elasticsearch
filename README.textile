...
We need to have a jdk-8 installed or adoptopenjdk-1.8 installed using Homebrew. 
set JAVA_HOME everytime we start a new terminal using @export JAVA_HOME=/Library/Java/JavaVirtualMachines/adoptopenjdk-1.8.jdk/Contents/Home@ 

h3. Installation

installation can be done by just cloning this repository ,switching to 5.4 branch by writing @git checkout 5.4@ in the terminal, after that we can run 
...
h3. Another Cluster Setup in 5.4 Version

change @http.port@ or @transport.tcp.port@ for setting up another cluster then just change it to 9201 and 9301 in elasticsearch.yml file at path @distribution/build/cluster/run node0/elasticsearch-5.4.4-SNAPSHOT/config/elasticsearch.yml@ just before attaching debugger to port

If the 8000 port is preoccupied by another Cluster Debugger then just add @-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:5006@ line in the @distribution/build/cluster/run node0/elasticsearch-5.4.4-SNAPSHOT/config/jvm.options@ and attach debugger to port 5006



h1. Elasticsearch 5.4
